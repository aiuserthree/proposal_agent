...
| PART | 섹션 | 상세 지침 |
|------|------|-----------|
| PART 2 | 2.1 시장·산업 개요 | **거시 시장 수치**(출처·연도) 표 1+; 국내·산업 맥락 문단; 검색·개인정보·접근성 등 **플랫폼·규제 시사** 1문단 이상. |
| PART 2 | 2.2 트렌드 | **기술·플랫폼 / UX·전환 / 운영·데이터** 등 **3축 이상**으로 소표 또는 소절 분리. 표마다 **제안 연결** 열. `trend-research` 스킬 활용 가능. |
| PART 2 | 2.3 경쟁·벤치마킹 | **3~5개** 사이트, **선정 이유 표** + (선택) **제외 후보 요약**. 사이트별 **반페이지~1p**: IA·내비, 검색, 성능·기술 체감, 콘텐츠·IR, 접근성·신뢰, **귀사 TO-BE 시사점**. **비교 매트릭스 2개 이상**. **RFP §/REQ 매핑 표 1개 이상**. URL·스크린샷 경로 필수. |
| PART 2 | 2.4 AS-IS 진단 | URL 또는 유사사례 추론 근거, 갭 표. **리뉴얼·고도화 RFP**에서는 `proposal-structure` §16에 따라 **UX/UI·콘텐츠·디자인** 관점 **분리 표** 후 통합 갭·우선순위. RFP **정량 지표** 재인용. |
| PART 2 | 2.5 시사점 | PART 1 과제 ID, **평가 항목**(이해도·UI/UX 등), PART 3 키워드로 이어지는 **행동 문장**. Mermaid 1개 권장. |

### 2.1 최종 제안서에 옮길 때의 최소 구성 (체크)

- 표 **5개 이상**(시장·트렌드·선정·매트릭스×2·AS-IS/갭 등 조합).  
- 벤치마킹 절(2.3)만으로 **4p 이상** 분량이 되도록 사이트별 서술을 채운다.

## 3. 섹션당 필수 포함 요소

- **2.1~2.2**: 수치·출처(기관·보고서·언론). 환각 금지. 수치 없으면 **“공개 수치 없음”** + 정성만.
- **2.3**: 비교 매트릭스 **2개 이상**(통합+세부). **RFP 매핑 표** 1개 이상. 스크린샷 경로 `workspace/drafts/screenshots/{사이트명}/`. research-agent **`run-benchmark`** 등 산출 경로를 초안 상단 메타에 기재.
- **2.4**: 이슈 ID, 우선순위(P0/P1), TO-BE 연결 한 줄. 갭마다 **벤치마킹 인사이트** 열(가능 시).
- **2.5**: "따라서 본 제안에서는…" 형태의 **행동 연결** + **평가 항목** 언급 1회 이상.
...
- PART 2 구간 **최소 5p·권장 8~14p**(writing-style **50p 표준**). 그중 **벤치마킹(2.3)·비교만 최소 4p** 이상.
- `competitor-analysis` 스킬 또는 동등 절차로 **3~5개 URL**·**매트릭스 2개 이상**·**RFP 매핑 표**를 반드시 포함한다.
- 부족 시 **시장 표·트렌드 축 추가·사이트별 소절·검색 쿼리 각주**로 보강한다.
...
- 담당 PART에서 **표 5개 이상**(시장·트렌드·선정·비교 매트릭스·AS-IS/갭 등). **비교 매트릭스만 2개 이상**.
