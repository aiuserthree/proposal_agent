...
## 10a. PART 3 서술·참고 자료 활용 (`writing-style` §1.1d · `proposal-structure` §18·§19)

- PART 3는 **표·키워드 나열만**으로 끝내지 않는다. **3.0 또는 3.1**에 경영진용 **한 줄 메시지**(Why)를 두고, 각 하위 절에 **문단 2~4개**로 근거·KPI·거버넌스를 서술한다.
- 사용자가 `img/` 등에 **참고용 슬라이드·캡처**를 두었을 때는 **최종 제안서에 이미지를 삽입하지 않는다.** 대신 **그 자료가 쓴 방식**(절 구조, 비교·그룹화, 표 vs 서술 비중, 시간순·매트릭스·다이어그램 선택)만 벤치하여, 동일 정보는 **표·Mermaid·문단**으로 옮긴다. 타사명·제품명은 본문에 인용하지 않는다.
- 타사명·수치·제품명을 **귀사 실적으로 오인**할 수 있게 쓰지 않는다.
