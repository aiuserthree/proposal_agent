---
name: admin-spec-writer
description: 관리자 시스템 명세
---
# admin-spec-writer

## 1. 목적과 컨텍스트

**백오피스·관리자** 기능을 FO와 분리해 명세한다. IA, 상세 기능, **권한 매트릭스**, 대시보드 레이아웃을 포함해 `developer`·`planner`가 동일 구조를 따름.
...
| 항목 | 내용 |
|------|------|
| 경로 | `workspace/drafts/admin-spec-{YYYY-MM-DD}.md` |
| 형식 | 표 중심 + 권한 매트릭스 + 대시보드 와이어 설명 |

## 4. 단계별 실행 절차 (6단계)

1. **역할**: 사용자 유형별 책임을 정의한다.
2. **메뉴**: BO IA와 일치하는 메뉴 트리를 확정한다.
3. **기능**: 메뉴별 CRUD·승인·배치·알림을 상세화한다.
4. **권한**: 행동(읽기/쓰기/삭제/다운로드) × 역할 표를 만든다.
5. **대시보드**: 위젯·지표·갱신 주기를 설명한다.
6. **보안**: IP 제한·2FA 등 RFP 요구를 반영한다.
...
| 상황 | 처리 |
|------|------|
| 역할 미정 | RFP 기본 3역할 제안 + 확인 |
| 권한 과다 | 최소 권한 원칙 문구 추가 |
