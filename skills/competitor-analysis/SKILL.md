...
**분량(필수)**: `writing-style` **1.1b·1.1c**에 따라 PART 2 중 **벤치마킹·비교만 최소 4페이지 분량**(인쇄 기준 환산)을 채운다. 최종 제안서 전체는 **50p+**, PART 2는 **5p+·권장 8~14p**가 되도록 본 스킬 산출이 **표 5개 이상·매트릭스 2개 이상·사이트별 반페이지~**를 포함한다.
...
| 형식 | 선정·제외 요약 + **비교 매트릭스 2개 이상** + **RFP 매핑 표** + 사이트별 반페이지~1p 요약 |
...
1. 최소 **3개**, 최대 **5개** 사이트.
2. 각 사이트 **출처 URL** 필수. **선정 이유** 표 필수. (가능 시) **제외 후보 1~2건** + 탈락 사유.
3. 표 **2개 이상**: ① **통합 비교**(상위 요약) ② **세부 축**(검색·CTA·성능·문서·접근성 등 과제 맞춤). 추가로 **RFP 벤치 패턴 ↔ §/REQ** 매핑 표 1개.
4. 사이트별 **IA·내비 / 검색 / 성능·기술 / 콘텐츠·IR / 접근성·신뢰 / 귀사 적용** 중 최소 **4축**을 서술하고, **최소 3~6문단**(또는 소제목+표)으로 **반페이지 이상** 분량 확보.
5. writing-style 금지 표현 사용 안 함.
6. MCP 실패 시 한계를 명시하고 **web_search**로 공개 정보 보강.
7. 참고 제안서 md가 있으면 **비교 항목 목록**(목차에서)을 매트릭스 열로 반영.
8. research-agent **`run-benchmark`** 결과가 있으면 **산출 경로**를 메타에 기록.
...
## 비교 매트릭스 A (통합)
| 항목 | A | B | C | 귀사 TO-BE |
## 비교 매트릭스 B (세부: 검색·성능·문서 등)
| 항목 | A | B | C |
## RFP 매핑
| RFP § 또는 REQ | 벤치에서 관찰한 패턴 | 본 제안 반영 |
## 사이트별 상세
### A — 이름
- IA·내비: …
- 검색: …
