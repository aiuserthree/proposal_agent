---
name: ia-generator
description: 정보구조(IA) 자동 생성
---
# ia-generator

## 1. 목적과 컨텍스트

프로젝트 유형에 맞는 **FO(프론트)·BO(백오피스)** 정보구조를 Mermaid로 생성하고 RFP 기능과 **1:1 매핑** 가능한 ID 체계를 제안한다. `planner`의 핵심 입력이 된다.
...
| 필수 | REQ 또는 기능 목록 | 예 |
| 필수 | FO/BO 분리 여부 | 예 |
| 선택 | 기존 사이트 메뉴(AS-IS) | 아니오 |

## 3. 출력 명세

| 항목 | 내용 |
|------|------|
| 경로 | `workspace/drafts/ia-{YYYY-MM-DD}.md` |
| 형식 | Mermaid `flowchart` + 메뉴 설명 표 |

## 4. 단계별 실행 절차 (7단계)

1. 사용자·관리자 여정을 구분한다.
2. **1차 메뉴**를 RFP 우선순위로 정렬한다.
3. 하위 메뉴를 기능 단위로 묶는다.
4. Mermaid 노드 ID는 영문으로, 라벨은 한글로 한다.
5. 각 메뉴에 **기능 ID** 열을 붙인 표를 만든다.
6. 추가 제안 메뉴는 `[추가제안]` 태그.
7. 파일 저장.
...
1. RFP 필수 기능 누락 없음.
2. Mermaid가 렌더 가능한 복잡도.
3. 메뉴명이 RFP 용어와 최대한 일치.
4. BO는 권한·감사 로그 등 운영 항목을 포함.
5. output-format Mermaid 규칙 준수.
...
| 상황 | 처리 |
|------|------|
| 기능 과다 | 2단계 다이어그램으로 분할 |
| RFP 모호 | 가정 표시 후 consensus에 연결 |
