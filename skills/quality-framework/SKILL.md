...
description: 품질/위험 관리 체계
---
# quality-framework

## 1. 목적과 컨텍스트

산출물·검토·테스트·리포트·하자보수·SLA를 **체계적으로** 기술한다. `pm-agent`의 품질·위험 절과 동일 톤으로 맞춘다.
...
| 항목 | 내용 |
|------|------|
| 경로 | `workspace/drafts/quality-risk-{YYYY-MM-DD}.md` |
| 형식 | 산출물 목록 + QA 프로세스 + 리스크 매트릭스 + SLA 표 |

## 4. 단계별 실행 절차 (6단계)

1. **산출물 목록**: 단계별 산출물·형식·검수자를 정의한다.
2. **검토 주기**: 설계·구현·이행 단계별 리뷰를 명시한다.
3. **테스트 전략**: 단위·통합·UAT 범위를 정한다.
4. **품질 지표**: 결함 밀도·재작업률 등 선택 지표를 적는다.
5. **리스크**: 식별·영향·대응·담당을 표로 정리한다.
6. **하자·유지보수**: 기간·응답 시간·채널을 SLA 형식으로 정한다.

## 5. 품질 기준

1. RFP SLA 수치가 있으면 **동일 단위**로 반영.
2. 리스크 최소 5건 또는 "해당 없음" 근거.
3. 테스트·UAT가 일정(schedule-planner)과 연결.
4. 용어가 developer·planner와 일치.
5. 표 2개 이상.
