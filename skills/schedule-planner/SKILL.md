...
description: WBS 일정 계획
---
# schedule-planner

## 1. 목적과 컨텍스트

프로젝트 **마일스톤·WBS·병행 작업**을 정의하고 `pm-agent`의 수행계획에 삽입한다. 규모(소·중·대) 기준은 **금액·기간**을 RFP와 맞춘다.

## 2. 입력 명세

| 구분 | 항목 | 필수 |
|------|------|------|
| 필수 | 계약 기간 또는 목표 오픈일 | 예 |
| 필수 | 필수 산출물(착수·중간·최종) | 예 |
| 선택 | 규모 구간(소 3억 미만 등) | 권장 |

## 3. 출력 명세

| 항목 | 내용 |
|------|------|
| 경로 | `workspace/drafts/schedule-wbs-{YYYY-MM-DD}.md` |
| 형식 | WBS 표 + Mermaid gantt 또는 간트 표 |

## 4. 단계별 실행 절차 (6단계)

1. 단계(착수·분석·설계·구현·테스트·이행)를 정의한다.
2. 각 단계에 **산출물·책임 역할**을 매핑한다.
3. 의존 관계(선행 작업)를 식별한다.
4. 기간을 배분하고 버퍼를 포함한다.
5. 마일스톤 날짜를 확정한다.
6. 리스크가 높은 작업(연동·데이터 이관)을 표시한다.

## 5. 품질 기준

1. RFP 제출일·착수일과 **모순 없음**.
2. 테스트·UAT 기간이 **0이 아님**.
3. 병합 가능한 작업은 병렬로 표시.
4. 표 또는 Gantt **1개 이상**.
5. pm-agent의 조직·품질 절과 날짜 일치.
...
| 상황 | 처리 |
|------|------|
| 기간 과소 | 버퍼·범위 축소 옵션을 주석 |
| 일정 미기재 | 업계 표준 비율로 가정 + 전제 명시 |
