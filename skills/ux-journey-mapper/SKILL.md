...
description: UX 여정 맵 작성
---
# ux-journey-mapper

## 1. 목적과 컨텍스트

**Awareness → Consideration → Purchase/전환 → Retention** 관점에서 터치포인트·행동·감정·개선 기회·KPI를 매핑한다. `planner`·`marketer`(유형 D/E)와 연계된다.

## 2. 입력 명세

| 구분 | 항목 | 필수 |
|------|------|------|
| 필수 | 페르소나(또는 RFP 이용자 유형) | 예 |
| 필수 | 필수 시나리오 2개 이상 | 예 |
| 선택 | 채널(웹·앱·SNS) | 아니오 |
...
| 항목 | 내용 |
|------|------|
| 경로 | `workspace/drafts/ux-journey-{YYYY-MM-DD}.md` |
| 형식 | 단계별 표 + 감정 곡선 설명 + KPI 표 |

## 4. 단계별 실행 절차 (6단계)

1. 페르소나 목표와 장애물을 정의한다.
2. 단계별 **터치포인트**(채널·URL·화면)를 나열한다.
3. 사용자 행동·시스템 반응을 분리해 기술한다.
4. 감정(긍정/중립/부정)을 태그한다.
5. 개선 기회를 기능 ID와 연결한다.
6. KPI·측정 방법을 표로 정리한다.
...
2. KPI는 **정량** 가능한 형태로.
3. RFP 서비스 목적과 정합.
4. 표 또는 Mermaid 중 1개 이상 포함.
5. writing-style 금지어 배제.
...
| 상황 | 처리 |
|------|------|
| 페르소나 미제공 | RFP 이용자 정의를 인용해 임시 생성 |
| B2B/B2C 혼동 | 별도 여정으로 분리 |
